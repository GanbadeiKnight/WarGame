<template>
	<div class="hexmap">
		...
		<div class="dialog" v-show="textIndex < (myText[turn]===undefined?0:myText[turn].length)" @click="nextText">
			...
			<img src="../../static/UI/stalin.png" v-show="turn===11&&textIndex===0?true:false">
		...
		<div class="terrain" v-for="(cell,unitIndex) in cellarr" v-if="cell.terrain>0"
			...
			:class="{ terrain_capital: cell.terrain === 30 }">
		...
		<div class="save" @click="saveFun">
			保存
		</div>
		<div class="load" @click="loadFun">
			载入
		</div>
...
<script>
	...
	const HexMapMixin = {
		...
		computed: {
			status() {
				...
				for (let i = 0; i < this.unitarr.length; i++) {
					if (this.unitarr[i].team === 0 && this.unitarr[i].hasUnit === true) {
						team0Count++
					} else if (this.unitarr[i].team === 1 && this.unitarr[i].hasUnit === true) {
						team1Count++
					}
				}
				if (team0Count > team1Count) {
					return '陷入劣势'
				} else if (team0Count < team1Count) {
					return '具有优势'
				} else if (team0Count > team1Count + 5) {
					return '岌岌可危'
				} else {
					return '势均力敌'
				}
		...
		mounted() {
			...
			this.WarAudio = new Audio('../../../static/audio/WarinDanger.mp3')
			this.bigactionAudio = new Audio('../../../static/audio/bigaction.mp3')
			...
			this.cellarr[66].tag = '莫扎伊斯克'
			this.cellarr[66].building = 9
			//为特定地块添加地形样式
		...
		methods: {
			...
			selectUnit(event, unit, attackStatus) {
				...
				if (unit.status === 'low_morale') {
					...
					unit.unitInfo.speed = getUnitInfo(unit.unitType).speed
				}
			...
			nextText() {
				this.textIndex++
				const selectAudio = new Audio('../../../static/audio/select.wav')
				selectAudio.volume = 0.2
				selectAudio.play()
				if (this.turn === 0 && this.textIndex === 1) {
					//播放背景bgm
					this.bigactionAudio.volume = 0.8
					this.bigactionAudio.play()
				}
				if (this.turn === 4 && this.textIndex === 1) {
					//切换音乐
					this.bigactionAudio.pause()
					this.bigactionAudio.currentTime = 0
					this.bigactionAudio = null
					setTimeout(() => {
						this.WarAudio.volume = 0.8
						this.WarAudio.play()
					}, 3000)

				}
				if (this.textIndex >= (this.myText[this.turn] === undefined ? 0 : this.myText[this.turn].length)) {
					console.log("初步验证完成")
					if (this.turn === 4) {
						const trackAudio = new Audio('../../../static/audio/track.wav')
						trackAudio.volume = 0.6
						trackAudio.play()
						this.product('tank_de', 97, 0).unitInfo.tag = "德国中央集团军群-南线"
						this.product('tank_de', 98, 0).unitInfo.tag = "德国中央集团军群-南线"

					}
					if (this.turn === 10) {
						const zhiyuanAudio = new Audio('../../../static/audio/zhiyuan.wav')
						zhiyuanAudio.volume = 0.6
						zhiyuanAudio.play()
						this.product('tank_su', 19, 1).unitInfo.tag = "列宁格勒方面军-后备部队"
						this.product('tank_heavy_su', 8, 1).unitInfo.tag = "列宁格勒方面军-后备部队"
						this.product('tank_su', 18, 1).unitInfo.tag = "列宁格勒方面军-后备部队"
						this.product('katyusha', 9, 1).unitInfo.tag = "列宁格勒方面军-后备部队"
						this.product('infantry_s_su', 28, 1).unitInfo.tag = "列宁格勒方面军-后备部队"
						this.product('katyusha', 29, 1).unitInfo.tag = "列宁格勒方面军-后备部队"
					}
				}
			},
			saveFun() {
				...
				this.username = sessionStorage.getItem('username')
				const indexObj = Object.fromEntries(this.indexMap)
				// 构造请求体数据
				const requestBody = {
					username: this.username,
					unitData: this.unitarr,
					turnsData: this.turn,
					indexMap: JSON.stringify(indexObj)
				}
				axios.post('/savegame', requestBody)
					.then(response => {
						// 请求成功，输出响应消息
			...
			loadFun() {
				...
				axios.get(`/loadgame/${username}`)
					.then(response => {
						...
						this.unitarr = []
						this.unitarr = cloneDeep(saveData.unitarr)
						this.turn = saveData.turns
						const indexobj = cloneDeep(saveData.indexMap)
						this.indexMap = new Map(Object.entries(indexobj))
						const newMap = new Map();

						// 遍历 this.indexMap 中的所有键值对
						this.indexMap.forEach((value, key) => {
							// 将字符型的键转换为数字型
							const numericKey = parseInt(key, 10);

							// 在新的 Map 对象中存储数字型的键和相应的值
							newMap.set(numericKey, value);
						})
						...
						this.unitarr.forEach((unit) => {
							const cell = this.cellarr[unit.id];
							if (unit.hasUnit) {
							cell.hasUnit = true;
							}
						})
				...
				AIfun.bind(this)().then(() => {
					setTimeout(() => {
						...
						if (this.turn === 10) {
							...
							this.WarAudio = null
							setTimeout(() => {
								const fightBack = new Audio(
									'../../../static/audio/fightBack.mp3')
								fightBack.volume = 0.8
								fightBack.play()
							}, 1500)
						}
						if (this.turn === 6) {
							this.unitarr.forEach(unit => {
								if (unit.team === 1 && unit.status === 'low_morale') {
									unit.status = ''
								}
							})
						}
